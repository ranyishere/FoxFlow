...
torch::Tensor calculate_unit_vector(double x1, double y1, double x2, double y2){double dx = (x2 - x1);double dy = (y2 - y1);double len = (sqrt((std::pow(dx, 2) + std::pow(dy, 2))));return torch::tensor({dx / len, dy / len}, torch::kFloat64);}
;
